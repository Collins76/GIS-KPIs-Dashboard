...
export default function KpiSummaryCards({ kpis }: KpiSummaryCardsProps) {
  ...
  const inProgress = kpis.filter((kpi) => kpi.progress > 0 && kpi.progress < 100 && kpi.status === 'On Track').length;
  ...
  const summaryData = [
    { title: 'Total KPIs', value: totalKpis, icon: BarChart, color: 'text-blue-400', progressColor: 'from-blue-500 to-blue-400', subText: 'All Active', subIcon: TrendingUp },
      ...
      {summaryData.map((item, index) => (
        <div key={index} className="kpi-card">
          ...
          <div className="w-full bg-gray-700 rounded-full h-2">
            <div 
              className={`bg-gradient-to-r h-2 rounded-full ${item.progressColor}`} 
